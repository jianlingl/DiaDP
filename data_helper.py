...
def load_annoted_inter(data_file, max_len, sparse=False):
    with open(data_file, 'r', encoding='utf-8') as f:
        data = json.load(f)
    ...
    for d in data:
        ...
        for tripple in d['relationship']:
            ...
            tail_uttr_idx, tail_word_idx = [int(x) for x in tail.split('-')]
            if head_uttr_idx != tail_uttr_idx:
                inters.append([head_word_idx + head_uttr_idx * max_len, rel, tail_word_idx + tail_uttr_idx * max_len])
                continue
...
class DialogUttrInterDataset(Dataset):
    ...
    def read_data(self):
        ...
        sem_roles = []
        speakers = []
        max_len = self.cfg.dialog_max_length   # per utterance

        sample_lst, inters_lst, speakers_lst = load_annoted_inter(self.data_file, max_len)

        sample_lst_aug, inters_lst_aug, speakers_lst_aug = [], [], []
        if not self.test:
            for file_path in self.cfg.aug_files:
                sa_lst, in_lst, sp_lst = load_annoted_inter(file_path, max_len, sparse=False)
